 ...
 * 2. Ejecuta: npx tsx scripts/migrate_to_supabase.ts
 */
...
async function migrateStudents() {
  ...
  const studentsData = JSON.parse(fs.readFileSync(studentsFile, 'utf-8')) as Student[];
...
async function migrateDetentions() {
  ...
  const detentionsData = JSON.parse(fs.readFileSync(detentionsFile, 'utf-8')) as Detention[];
